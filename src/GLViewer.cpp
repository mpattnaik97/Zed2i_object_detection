...
void GLViewer::updateView(sl::Mat image, sl::Objects &objs, int fps)
{
	mtx.lock();
    // Update Image
...
void GLViewer::printText() {
	...
	for (auto it : objectsName) {
		...
		glWindowPos2f(pt2d.x, std::min(static_cast<size_t>(pt2d.y), wnd_size.height - 50));
...
void GLViewer::printText(std::string text)
{
	sl::Resolution wnd_size(glutGet(GLUT_WINDOW_WIDTH), glutGet(GLUT_WINDOW_HEIGHT));
	glDisable(GL_BLEND);
	...
	glWindowPos2f(50, wnd_size.height - 50);
	int len = (int)strlen(string);
	for (int i = 0; i < len; i++)
		glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, string[i]);
